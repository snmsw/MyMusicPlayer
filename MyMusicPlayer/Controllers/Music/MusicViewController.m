...
@property (nonatomic, strong) MusicIndicator *musicIndicator;
@property (nonatomic, strong) NSMutableArray *originArray;
@property (nonatomic, strong) NSMutableArray *randomArray;
@property (nonatomic, strong) NSMutableArray *lastMusicUrl;
...
{
    UISwipeGestureRecognizer *swipRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(didTouchDismissButton:)];
    swipRecognizer.direction = UISwipeGestureRecognizerDirectionDown;
...
- (IBAction)didTouchMusicCycleButton:(id)sender {
    switch (_musicCycleType) {
        case MusicCycleTypeLoopAll: {
            ...
            [self showMiddleHint: @"随即播放"]; } break;
        case MusicCycleTypeShuffle: {
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(updatePlaybackIndicatorOfVisibleCells)]) {
        [_delegate updatePlaybackIndicatorOfVisibleCells];
    }
}
